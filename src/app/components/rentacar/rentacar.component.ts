...
export class RentacarComponent implements OnInit {
  ...
  createRental() {
    let MyRental: Rental = {
      ...
      returnDate: this.returnDate ? this.rentDate : null,
    ...
    if (this.checkAvailability() == true) {
      if (this.rentDate == null) {
        this.toastrService.warning(
          'Başlangıç tarihi ve şirket seçimi zorunludur!',
          'Eksik Form'
        );
      } else {
        if (this.rentDate == null || this.rentDate > this.rentDate) {
          this.toastrService.success('Araç kiralanabilir.', 'Araç Uygun');
          this.createRental();
        } else if (this.rentDate < this.rentDate) {
          this.toastrService.error(
            'Donüş tarihi başlangıç tarihinden küçük olamaz!'
          );
